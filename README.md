...
- **Docked panel** — an optional panel attached to the character sheet,
  ...
  missing. A checkbox on the character sheet turns it off; drag its title
  (or the grip in its top-left corner) to move it anywhere, and the grip
  in its bottom-right corner resizes it (right-click either grip to
  put it back).
